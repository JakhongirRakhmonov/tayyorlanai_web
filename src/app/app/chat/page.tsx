...
export default function ChatPage() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-140px)]">
      <div className="bg-white rounded-t-2xl p-4 shadow-sm border-x border-t">
      ...
      <div className="flex-1 overflow-y-auto bg-white border-x px-4 py-4 space-y-3">
        {msgs.length === 0 && (
        ...
        {msgs.map((m, i) => (
          <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
            <div className={`max-w-[85%] px-4 py-3 rounded-2xl text-sm whitespace-pre-wrap ${
              m.role === "user" ? "bg-primary-600 text-white rounded-br-md" : "bg-gray-100 text-gray-800 rounded-bl-md"
            }`}>
              {m.text}
            </div>
          </div>
      ...
      <div className="bg-white rounded-b-2xl p-3 border shadow-sm flex gap-2">
        <input value={input} onChange={(e) => setInput(e.target.value)}
          onKeyDown={(e) => e.key === "Enter" && !e.shiftKey && send()}
          placeholder="Savolingizni yozing..."
          className="flex-1 px-4 py-2.5 border rounded-xl text-sm focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none" />
