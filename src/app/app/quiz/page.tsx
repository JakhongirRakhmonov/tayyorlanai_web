  ...
  if (showAnalysis) {
    return (
      <div className="space-y-4 animate-fade-in">
        ...
        {/* Progress bar */}
        <div className="bg-white dark:bg-gray-900 rounded-2xl p-4 shadow-sm border dark:border-gray-800">
            ...
            <div key={qi} className={`rounded-2xl p-5 shadow-sm border animate-slide-up ${isCorrect ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"}`}
              ...
              <div className="space-y-1.5 ml-9">
                {q.options.map((opt, oi) => {
                  let cls = "text-gray-400 dark:text-gray-500 text-xs";
                  ...
                  else cls = "text-gray-400 dark:text-gray-500 text-xs px-3 py-1";
                  return (
    ...
    <div className="space-y-4 animate-fade-in">
      <div className="bg-white dark:bg-gray-900 rounded-2xl p-5 shadow-sm border dark:border-gray-800">
        ...
        {questions.length === 0 && !loading && (
          <>
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">Nechta savol yaratilsin?</p>
      ...
      {loading && (
        <div className="bg-white rounded-2xl p-10 shadow-sm border text-center animate-scale-in">
          ...
          <p className="text-gray-500 dark:text-gray-400 font-medium">Test savollari yaratilmoqda...</p>
        </div>
      )}
      ...
      {questions.length > 0 && (
        <div className="space-y-4">
          ...
          {questions.map((q, qi) => {
            ...
            return (
              <div key={qi} className={`bg-white dark:bg-gray-900 rounded-2xl p-5 shadow-sm border dark:border-gray-800 transition-all animate-slide-up ${!submitted && qi === current ? "ring-2 ring-primary-200" : ""}`}
                style={{ animationDelay: `${qi * 50}ms`, animationFillMode: 'both' }}>
                <p className="font-medium mb-3 text-sm">
                  <span className="inline-block w-7 h-7 bg-gray-100 dark:bg-gray-800 rounded-lg text-center leading-7 text-xs font-bold mr-2">{qi + 1}</span>
                  {q.question}
                </p>
                  ...
                  {q.options.map((opt, oi) => {
                    ...
                    return (
                      <button key={oi} disabled={isAnswered}
                        ...
                        <span className="inline-block w-6 h-6 rounded-lg bg-gray-100 dark:bg-gray-700 text-center leading-6 text-xs font-bold mr-2">{labels[oi]}</span>
                        {opt}
